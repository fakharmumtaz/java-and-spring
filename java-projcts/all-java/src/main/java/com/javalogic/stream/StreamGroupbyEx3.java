package com.javalogic.stream;

import com.javalogic.model.Item;

import java.util.Arrays;
import java.util.List;
import java.util.Map;
import java.util.stream.Collectors;

public class StreamGroupbyEx3 {
    public static void main(String[] args) {

        //3 apple, 2 banana, others 1
        List<Item> items = Arrays.asList(
                new Item("apple", 10, new Double("9.99")),
                new Item("banana", 20, new Double("19.99")),
                new Item("orang", 10, new Double("29.99")),
                new Item("watermelon", 10, new Double("29.99")),
                new Item("papaya", 20, new Double("9.99")),
                new Item("apple2", 10, new Double("12.0")),
                new Item("banana", 10, new Double("19.99")),
                new Item("apple2", 20, new Double(10.0d))
        );

        Map<String, Long> counting = items.stream().collect(
                Collectors.groupingBy(Item::getName, Collectors.counting()));

        System.out.println(counting);

        Map<String, Integer> totQty = items.stream().collect(
                Collectors.groupingBy(Item::getName, Collectors.summingInt(Item::getQty)));

        System.out.println(totQty);

        Map<String, Double> totalPrice = items.stream().collect(
                Collectors.groupingBy(Item::getName, Collectors.summingDouble(Item::getPrice)));

        System.out.println(totalPrice);

    }

}